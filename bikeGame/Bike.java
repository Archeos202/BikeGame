...
public class Bike extends GameEntity implements Actor  {
	...
	public void update(float deltaTime) {
		if (getOwner().getKeyboard().get(KeyEvent.VK_ESCAPE).isDown()) {
			if (regard = true) {
				regard = false;
			}
			else {
		...
		if ((regard=true) && (leftWheel.getSpeed()> -MAX_WHEEL_SPEED)) {
			leftWheel.power(20.0f);
		}
		if ((regard=false) && (rightWheel.getSpeed()< MAX_WHEEL_SPEED)) {
			rightWheel.power(20.0f);
		}
